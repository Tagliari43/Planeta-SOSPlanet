      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 w-[calc(100vw-3rem)] sm:w-[380px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-100"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-green-800 to-green-600 p-4 flex flex-col text-white">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm shadow-inner relative">
                    ...
                    <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-400 border-2 border-green-700 rounded-full"></span>
                  </div>
                  <div>
              ...
              {/* Tabs */}
              <div className="flex bg-green-900/40 rounded-lg p-1 gap-1">
                <button
            ...
            {/* Content Area */}
            <div className="flex-1 bg-gradient-to-b from-gray-50 to-white flex flex-col relative h-[400px]">
              {activeTab === 'chat' ? (
                <>
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    <AnimatePresence initial={false}>
                          ...
                          <div
                            className={cn(
                              ...
                              msg.role === 'user'
                                ...
                                : "bg-white/80 backdrop-blur-md border border-green-100 text-gray-800 rounded-bl-sm"
                            )}
                          >
                            <p className="leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                            <span className={cn(
                              "text-[9px] mt-2 block font-medium",
                              msg.role === 'user' ? "text-green-200 text-right" : "text-gray-400"
                            )}>
                      ...
                      {isTyping && (
                        ...
                        >
                          <div className="bg-white/80 backdrop-blur-md border border-green-100 rounded-2xl rounded-bl-sm p-4 shadow-sm flex items-center gap-1.5">
                  ...
                  <div className="p-3 bg-white border-t border-gray-100">
                    <form onSubmit={handleSendMessage} className="relative flex items-center">
                      <input
                        ...
                        className="w-full bg-gray-50 border border-gray-200 rounded-full pl-4 pr-12 py-2.5 text-sm focus:outline-none focus:border-green-400 focus:ring-2 focus:ring-green-100 transition-all placeholder:text-gray-400"
                      />
                      <button
                        type="submit"
                        disabled={!inputValue.trim() || isTyping}
                        className="absolute right-1.5 w-8 h-8 flex items-center justify-center bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-full transition-colors"
                      >
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6 pb-12">
                  <div className="w-16 h-16 bg-green-50 rounded-full flex items-center justify-center mb-4 text-green-600 shadow-sm border border-green-100">
                    <Sparkles className="w-8 h-8 animate-pulse" />
                  </div>
                  
                  <p className="text-gray-700 italic font-medium leading-relaxed mb-6 text-sm">
                    "O canal direto com o núcleo está em construção. O Radar permanece ativo."
                  </p>

                  <div className="w-full bg-white p-4 rounded-xl shadow-sm border border-gray-100 mt-auto">
                    {!submitted ? (
                      <form onSubmit={handleSubmitEmail} className="flex flex-col gap-3">
                        <label htmlFor="emailInput" className="text-xs font-semibold text-gray-500 uppercase tracking-wider text-left pl-1">
                        ...
                        <div className="relative">
                          <input 
                            ...
                            className="w-full pl-4 pr-12 py-3 rounded-lg border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none transition-all text-sm"
                          />
                          <button 
                            type="submit"
                            className="absolute right-2 top-1/2 -translate-y-1/2 bg-green-600 hover:bg-green-700 text-white p-1.5 rounded-md transition-colors"
                          >
